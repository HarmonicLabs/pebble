...
- The full V4 script-context type family, mirroring
  **plutus-ledger-api 1.68.0.0** (the release that introduced
  `PlutusLedgerApi.V4` and `dijkstraPV = 12`): `ScriptContextV4` (4
  fields, incl. the executing script's own hash), `TxV4` (19 fields —
  no fee, no requiredSigners; adds `subTxIx`, `directDeposits`,
  `accountBalanceIntervals`, `guards`, `requiredTopLevelGuards`),
  `TxOutV4`/`TxInV4`/`AddressV4`, `POSIXTimeRange` (plain optional
  bounds), `AccountId`, `AccountBalanceInterval(s)`, the account-based
  `TxCertV4`, `ScriptPurposeV4` (script hash in every constructor, new
  `Guarding`), `ScriptInfoV4` (incl. `Guard` with the CIP-0118
  `TopTxInfo` whole-batch view) and `TopTxInfo(Simplified)`.
...
- The Dijkstra hard fork has not happened and CIP-0118 is still
  ...
  they are additive (`V4`-suffixed) and the V3 prelude is untouched.
